function varargout = Image_Crypt(P,para,K)
global time
%% 1. Initialization
...
switch para
    case 'encryption'
        for i = 1:iter
            ...
            ts0 = mod(log(turb(mod(i-1,6)+1)+i)*s0+T,1);
            xy = zeros(MN,2);
            for n = 1:MN
                if n == 1
            ...
            R = cat(3,reshape(xy(:,1),size(P,1),size(P,2)),reshape(xy(:,2),size(P,1),size(P,2)),reshape(s(:,1),size(P,1),size(P,2)));
